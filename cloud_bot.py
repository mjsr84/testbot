...
def load_models():
    try:
        ...
        models = [m["name"] for m in llm_response.json()["models"] if "cloud" in m["name"]]
...
# Conversation history is saved to a text file, which is read in full and sent with each prompt 
# to provide context. This is not the most efficient method for long conversations, 
# but it is the simplest way to maintain conversation history without using a database or 
# more complex data structure. Future iterations could implement a more efficient method of storing and 
# retrieving conversation history.
    if not os.path.exists("chat_history.txt"):
        new_chat_history = open("chat_history.txt", "x")
        new_chat_history.write("User: " + prompt_entry.get() + "\n" + model + ": " + llm_response.message.content + "\n")   
    else:
